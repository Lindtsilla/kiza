...
export class Tab1Page implements OnInit {
  ...
  @ViewChild('contactImport') contactImportPhone!: IonModal
  ...
  async ngOnInit() {
    ...
    this.allContact = JSON.parse(localStorage.getItem('contact') as string) as any[];
     this.allContact =  this.allContact ?  this.allContact : [];
  ...
  async getAllContactPhoneUuser() {
    ...
    this.contactImportPhone.present();
  ...
  returnData(): any[]{
   ...
   addContactPhone(){
    ...
    this.contactImportPhone.dismiss();
...
setDataView() {
  if(this.returnData()?.length > 0) this.allContact = this.returnData();
  else{
    this.allContact = [
      {
        name: 'Linda',
   ...
   async importContact() {
    ...
    if (permission) {
      ...
      let alert = this.alertCtrl.create({
        ...
        message: "Autoriser KIZA a acceder a vos contacts",
        buttons: [
          {
            text: 'Annuler',
            role: 'cancel',
